...
2. If the path exists and `bash ls` reads it, the session is in **Obsidian mode**: the vault is canonical for entity context (people, projects, meetings, decisions, observations).
3. Otherwise **Filesystem mode**: entity data lives in the repo folders (`team/`, `stakeholders/`, `projects/`, `meetings/`, `decisions/`).
