...
7. **Do not auto-write group-chat candidates** (they are lower-confidence than direct-thread observations). Instead, prompt the user:
   - **Interactive run:** after presenting the summaries, ask directly, "Want me to update the knowledge base with any of these?" and apply only the ones the user confirms (additive, via `edit_note`).
